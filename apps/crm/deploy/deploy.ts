...
/**
 * Super Simple CRM Deployment Script
 *
...
const APP_NAME = "super-simple-apps-crm";
...
const CRM_SUBDOMAIN = `crm.${ROOT_DOMAIN}`;
const DOMAIN_NAME = STAGE === "prod" ? CRM_SUBDOMAIN : "";
...
function setupLogging() {
  ...
  const logFile = join(logDir, `crm-${STAGE}-${timestamp}.log`);
  setLogFile(logFile);
...
async function uploadSchema(): Promise<string> {
  ...
  const schemaKey = `crm/schema-${schemaHash}.graphql`;
  ...
  if (deployRoleArn) {
    ...
    const assumeRoleResponse = await stsClient.send(
      new AssumeRoleCommand({
        ...
        RoleSessionName: "crm-frontend-deploy",
...
async function deploy(): Promise<void> {
  ...
  logger.info("Starting Super Simple CRM deployment");
...
async function remove(): Promise<void> {
  ...
  logger.info("Removing Super Simple CRM stacks");
  logger.info(`   Stage: ${STAGE}`);
